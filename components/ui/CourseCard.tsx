      ...
      <div className="rounded-2xl p-6 md:p-7 bg-gradient-to-br from-panel to-panel-2 text-white shadow-card ring-1 ring-gold/20 transition-all duration-300 md:hover:-translate-y-1 md:hover:shadow-card-hover flex flex-col h-full relative overflow-hidden order-first lg:order-none lg:scale-105 lg:z-10">
        ...
        <div className="flex items-start justify-between gap-3 mt-3 mb-4">
          ...
          <CourseInfo description={description}>
            <span
              className={`shrink-0 inline-flex items-center gap-1.5 bg-white/10 rounded-full px-2.5 py-1 ${description ? "ring-1 ring-white/25" : ""}`}
            >
              ...
              <span className="text-xs font-semibold text-white/90">{duration}</span>
            </span>
          </CourseInfo>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-x-3 gap-y-2 mb-5 flex-1">
          {features.map((feature, i) => (
            <div key={i} className="flex items-start gap-2 text-white/80 text-sm">
              <span className="shrink-0 mt-0.5 w-4 h-4 rounded-full bg-gold/20 flex items-center justify-center">
